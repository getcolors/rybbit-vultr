...
- Made the documented existing external identity explicit; the provider key reference and external mode are unchanged.

Outstanding live-operation prerequisites:

- The documented unmanaged SSH stanza remains an explicit conflict to resolve separately; this refresh neither reads private keys nor edits SSH configuration.
